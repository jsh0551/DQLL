...
def train(model, scaler, train_loader, val_loader, optimizer, lr_scheduler, start_epoch, no_aug):
    ...
    val_epoch, val_ap, val_ap50, val_ap75, val_ap_s, val_ap_m, val_ap_l = [], [], [], [], [], [], []
    val_loss, val_conf_loss, val_cls_loss, val_iou_loss = [], [], [], []
    ...
    for epoch in range(start_epoch + 1, opt.num_epochs + 1):
        ...
        if opt.val_intervals > 0 and epoch % opt.val_intervals == 0:
            ...
            logger.write("\n{}\n".format(r))
            val_epoch.append(epoch), val_ap.append(loss_dict_val["AP"]), val_ap50.append(loss_dict_val["AP_0.5"]), val_ap75.append(loss_dict_val["AP_0.75"])
            val_ap_s.append(loss_dict_val["AP_small"]), val_ap_m.append(loss_dict_val["AP_medium"]), val_ap_l.append(loss_dict_val["AP_large"])
            val_loss.append(loss_dict_val['loss']),val_conf_loss.append(loss_dict_val['conf_loss']),val_cls_loss.append(loss_dict_val['cls_loss']),val_iou_loss.append(loss_dict_val['iou_loss'])
            val_log = pd.DataFrame()
            val_log['epoch'] = val_epoch
            val_log['AP'] = val_ap
            val_log['AP_0.5'] = val_ap50
            val_log['AP_0.75'] = val_ap75
            val_log['AP_small'] = val_ap_s
            val_log['AP_medium'] = val_ap_m
            val_log['AP_large'] = val_ap_l
            val_log['loss'] = val_loss
            val_log['conf_loss'] = val_conf_loss
            val_log['cls_loss'] = val_cls_loss
            val_log['iou_loss'] = val_iou_loss
            val_log.to_csv(os.path.join(opt.save_dir,'val_log.csv'),index=False)
            if ap >= best:
