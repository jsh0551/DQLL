...
def random_hsv_transform(img_array, h_range, s_range, v_range):
        ...
        # 이미지를 HSV로 변환
    hsv_image = Image.fromarray(img_array.astype(np.uint8))
    hsv_image.convert('HSV')
    ...
    h += (random.uniform(-h_range, h_range) * 255)
    s += (random.uniform(-s_range, s_range) * 255)
    v += (random.uniform(-v_range, v_range) * 255)
    ...
    # 변환된 배열을 PIL 이미지로 다시 변환
    hsv_image = Image.merge('HSV', (Image.fromarray(h), Image.fromarray(s), Image.fromarray(v)))

    # 원래의 RGB 포맷으로 변환
    return np.array(hsv_image.convert('RGB')).astype(np.float32)
                ...
                for k, (qval, cur_point, gt_point, hist_vec, cur_x) in enumerate(zip(qvals, cur_points, gts, hist_vecs, cur_x_list)):
                    ...
                    if action != 4:
                        if action == 1:
                            ...
                            cur_point -= reward.get_dst(gt_point, cur_point)
                        elif action == 3:
                            cur_point += reward.get_dst(gt_point, cur_point)
                        cur_x.append(cur_point)
                    else:
